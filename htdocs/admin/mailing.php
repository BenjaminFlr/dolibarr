...
if ($action == 'setvalue') {
	...
	$res = dolibarr_set_const($db, "MAILING_LIMIT_SENDBYWEB", $batchlimit, 'chaine', 1, 'Number of targets to define packet size when sending mass email', $conf->entity); // note required because of the entity value
	if (!($res > 0)) {
		$error++;
	}
...
// Limit number for each mailing batch
print '<tr class="oddeven">';
